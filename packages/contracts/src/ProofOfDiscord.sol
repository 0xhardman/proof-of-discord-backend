...
contract ProofOfDiscord is ERC721Enumerable {
    using StringUtils for *;
    using NFTSVG for *;

    uint16 public constant bytesInPackedBytes = 31;
    string constant domain = "discord.com";
    
    uint32 public constant pubKeyHashIndexInSignals = 0; // index of DKIM public key hash in signals array
    uint32 public constant usernameIndexInSignals = 1; // index of first packed discord username in signals array
    uint32 public constant usernameLengthInSignals = 1; // length of packed discord username in signals array
    ...
    function tokenDesc(uint256 tokenId) public view returns (string memory) {
        string memory discord_username = tokenIDToName[tokenId];
        address address_owner = ownerOf(tokenId);
        string memory result = string(
            abi.encodePacked("Discord username", discord_username, "is owned by", StringUtils.toString(address_owner))
        );
        return result;
    }
    ...
    function _domainCheck(uint256[] memory headerSignals) public pure returns (bool) {
        ...
        string[2] memory domainStrings = ["verify@discord.com", "info@discord.com"];
        return
    ...
    /// Mint a token proving discord ownership by verifying proof of email
    /// @param proof ZK proof of the circuit - a[2], b[4] and c[2] encoded in series
    /// @param signals Public signals of the circuit. First item is pubkey_hash, next 3 are discord username, the last one is etherum address
    function mint(uint256[8] memory proof, uint256[3] memory signals) public {
        ...
        // Right now, we just check that any email was received from anyone at Discord, which is good enough for now
