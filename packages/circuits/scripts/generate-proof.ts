...
program
  ...
  .requiredOption(
    ...
    "Ethereum address to verify discord handle against"
  )
...
async function generate() {
  ...
  const circuitInputs = await generateDiscordVerifierCircuitInputs(rawEmail, args.ethereumAddress);
  ...
  const vkey = JSON.parse(fs.readFileSync(path.join(BUILD_DIR, `/artifacts/discord.vkey.json`)).toString());
