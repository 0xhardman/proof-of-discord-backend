...
/// @title DiscordVerifier
/// @notice Circuit to verify input email matches Discord password reset email, and extract the username
...
/// @input discordUsernameIndex Index of the Discord username in the email body.
...
template DiscordVerifier(maxHeadersLength, maxBodyLength, n, k, exposeFrom) {
    assert(exposeFrom < 2);
    ...
    // DISCORD REGEX: 328,044 constraints
    ...
    signal (discordFound, discordReveal[maxBodyLength]) <== DiscordResetRegex(maxBodyLength)(emailBody);
    discordFound === 1;

    // Pack the username to int
    var maxDiscordUsernameLength = 21;
    signal discordUsernamePacks[1] <== PackRegexReveal(maxBodyLength, maxDiscordUsernameLength)(discordReveal, discordUsernameIndex);
   
    // Username will fit in one field element, so we take the first item from the packed array.
    discordUsername <== discordUsernamePacks[0];
}


component main { public [ address ] } = DiscordVerifier(1024, 1536, 121, 17, 0);
